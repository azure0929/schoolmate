...
const apiService = {
  checkDuplicate: (type, value) => api.get(`/auth/check-${type}`, { params: { [type]: value } }),
  submitSignup: (data) => api.post("/auth/signup", data),
  submitSocialSignup: (data) => api.post("/auth/signup/social", data),
  login: (credentials) => api.post("/auth/login", credentials),
};

const allergyData = [
    { id: 1, name: "난류", icon: allergy1 }, { id: 2, name: "우유", icon: allergy2 },
    { id: 3, name: "메밀", icon: allergy3 }, { id: 4, name: "땅콩", icon: allergy4 },
    { id: 5, name: "대두", icon: allergy5 }, { id: 6, name: "밀", icon: allergy6 },
    { id: 7, name: "고등어", icon: allergy7 }, { id: 8, name: "게", icon: allergy8 },
    { id: 9, name: "새우", icon: allergy9 }, { id: 10, name: "돼지고기", icon: allergy10 },
    { id: 11, name: "복숭아", icon: allergy11 }, { id: 12, name: "토마토", icon: allergy12 },
    { id: 13, name: "아황산류", icon: allergy13 }, { id: 14, name: "호두", icon: allergy14 },
    { id: 15, name: "닭고기", icon: allergy15 }, { id: 16, name: "쇠고기", icon: allergy16 },
    { id: 17, name: "오징어", icon: allergy17 }, { id: 18, name: "조개류", icon: allergy18 },
    { id: 19, name: "잣", icon: allergy19 }, { id: 20, name: "아무개", icon: allergy20 },
];
...
const SignUp = () => {
    ...
    useEffect(() => {
        ...
        if (token && !tempToken) {
            ...
            setValidation(prev => ({ ...prev, email: { status: 'valid', message: '소셜 계정 이메일입니다.' } }));
        }
    }, [location, tempToken]);

    const isStep1NextDisabled = useMemo(() => {
        const { email, password, confirmPassword, name, nickname, birthDay, gender, phone } = formData;
        const requiredFieldsFilled = email && password && confirmPassword && name && nickname && birthDay && gender && phone;
        
        // [수정] '다음' 버튼 활성화 조건에 비밀번호 정책 유효성(validation.password.status) 추가
        const checksPassed = validation.email.status === "valid" &&
            ...
            (!!tempToken || (validation.password.status === "valid" && validation.passwordMatch.status === "valid"));
            
        return !(requiredFieldsFilled && checksPassed);
    }, [formData, validation, tempToken]);

    const isStep2NextDisabled = useMemo(() => {
        const { level, schoolName, majorName, grade, classNo } = formData.schoolInfo;
    ...
    const handleChange = (e) => {
        ...
        if (["email", "nickname", "phone"].includes(name)) {
            setValidation((prev) => ({ ...prev, [name]: { status: "unchecked", message: "" } }));
        }
        ...
        if (name === "password" || name === "confirmPassword") {
            ...
            const isMatch = newPassword && newConfirmPassword && newPassword === newConfirmPassword;
            setValidation(prev => ({
                ...prev,
                passwordMatch: { status: isMatch ? "valid" : "invalid", message: isMatch ? "" : "비밀번호가 일치하지 않습니다." }
            }));
        }
        ...
        if (name === "name" || name === "nickname") {
            ...
            let status = "unchecked", message = "";
    
            if (value.length > maxLength) {
                status = "invalid";
                message = `${fieldName}은(는) ${maxLength}자를 초과할 수 없습니다.`;
            } else if (/[^가-힣a-zA-Z0-9\s]/.test(value) && !isNameField) { // 닉네임 특수문자 체크
                status = "invalid";
                message = "닉네임에는 특수문자를 사용할 수 없습니다.";
            } else if (/[^가-힣a-zA-Z\s]/.test(value) && isNameField) { // 이름 숫자/특수문자 체크
                status = "invalid";
                message = "이름에는 숫자나 특수문자를 사용할 수 없습니다.";
            }
            setValidation(prev => ({ ...prev, [validationType]: { status, message } }));
        }
    };
    ...
    const handleDuplicateCheck = async (type) => {
        const typeKorean = { email: "이메일", nickname: "닉네임", phone: "전화번호" };
        ...
        setValidation((prev) => ({ ...prev, [type]: { status: "checking", message: "확인 중..." } }));
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
                setValidation((prev) => ({
                    ...prev,
                    [type]: { status: "invalid", message: `이미 사용 중인 ${typeKorean[type]}입니다.` },
                }));
            } else {
                setValidation((prev) => ({
                    ...prev,
                    [type]: { status: "invalid", message: "확인 중 오류가 발생했습니다." },
                }));
    ...
    const handleNormalSubmit = async (data) => {
        try {
            ...
            alert(`회원가입 중 오류 발생: ${error.response?.data?.message || "서버 오류"}`);
        }
    };
    ...
    const handleSocialSubmit = async (data) => {
        try {
            ...
            alert(`가입 중 오류 발생: ${error.response?.data?.message || "서버 오류"}`);
        }
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (tempToken) {
            const socialSignupData = { tempToken, student: { email: rest.email, password: rest.password, name: rest.name }, profile: commonProfileData, allergyId };
            handleSocialSubmit(socialSignupData);
        } else {
            const signupData = { student: { email: rest.email, password: rest.password, name: rest.name }, profile: commonProfileData, allergyId };
      ...
      <FormBox>
        ...
        <RequiredForm onSubmit={handleNextStep} $visible={step === 1}>
          ...
          <InputGroup>
            <InputWrapper>
              ...
              />
              {!tempToken && (
                <CheckButton type="button" onClick={() => handleDuplicateCheck("email")}>
                  중복확인
                </CheckButton>
              )}
            </InputWrapper>
          ...
          <InputGroup>
            <InputWrapper>
              <Icon><FaRegUser /></Icon>
              <Input type="text" name="nickname" placeholder="닉네임 (10자 이내, 특수문자 제외)" value={formData.nickname} onChange={handleChange} />
              <CheckButton type="button" onClick={() => handleDuplicateCheck("nickname")}>
                중복확인
              </CheckButton>
            </InputWrapper>
            <ValidationMessage $status={ validation.nickname.status !== "invalid" && validation.nicknamePattern.status === "invalid" ? "invalid" : validation.nickname.status }>
              {validation.nickname.status === "valid" ? validation.nickname.message : validation.nicknamePattern.message || validation.nickname.message}
            </ValidationMessage>
          </InputGroup>

          <InputGroup>
            <InputWrapper>
              <Icon><MdOutlinePermContactCalendar /></Icon>
          ...
          <InputGroup>
            <GenderButtonWrapper>
              <GenderButton type="button" selected={formData.gender === "남자"} onClick={() => handleGenderChange("남자")}>남자</GenderButton>
              <GenderButton type="button" selected={formData.gender === "여자"} onClick={() => handleGenderChange("여자")}>여자</GenderButton>
            </GenderButtonWrapper>
          </InputGroup>

          <InputGroup>
            <InputWrapper>
              <Icon><MdOutlinePhone /></Icon>
              <Input type="tel" name="phone" placeholder="휴대전화번호" value={formData.phone} onChange={handleChange} />
              <CheckButton type="button" onClick={() => handleDuplicateCheck("phone")}>
                중복확인
              </CheckButton>
            </InputWrapper>
        ...
        <RequiredForm onSubmit={handleNextStep} $visible={step === 2}>
          <RequiredSection>학교 정보 입력</RequiredSection>
           <SchoolSelector schoolData={formData.schoolInfo} onSchoolChange={handleSchoolDataChange} />
          <NextButton type="button" onClick={handleNextStep} disabled={isStep2NextDisabled}>다음</NextButton>
        </RequiredForm>
        ...
        <OptionalSection onSubmit={handleSubmit} $visible={step === 3}>
          ...
          <AllergyGrid>
            {allergyData.map((item) => (
              <AllergyItem key={item.id} onClick={() => handleAllergySelect(item.id)} selected={formData.allergyId.includes(item.id)}>
