...
const ActionConfirmModal = ({ type, onClose, onConfirm }) => {
  ...
  const title = isWithdrawal ? "정말 탈퇴 하시겠습니까?" : "로그아웃 하시겠습니까?";
  const confirmText = "예";
  ...
  return (
    <ModalWrapper ref={wrapperRef} style={{ opacity: 0 }}>
      <ModalContent ref={modalRef}>
        ...
        <ModalActions>
          <ConfirmButton $type={type} onClick={handleConfirm}>{confirmText}</ConfirmButton>
...
const ConfirmButton = styled.button`
  ${ModalButtonBase}
  background-color: ${props => props.$type === 'withdrawal' ? '#d9534f' : '#333'};
  color: white;
  &:hover { background-color: ${props => props.$type === 'withdrawal' ? '#c9302c' : '#555'}; }
`;
